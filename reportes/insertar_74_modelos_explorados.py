...
def set_tnr(run, size=12, bold=False):
...
def paragraph_before(anchor, text, style):
...
def main():
    doc = Document(DOCX)
    anchor = find_paragraph(doc, "Conclusiones")
    if anchor is None:
        raise RuntimeError("No se encontró el encabezado 'Conclusiones'.")

    for heading, paragraphs in reversed(BLOCKS):
        for text in reversed(paragraphs):
            paragraph_before(anchor, text, "Texto Normal TNR")
        paragraph_before(anchor, heading, "Título 3 TNR")

    doc.save(DOCX)
    print("Subsecciones 7.4.4 y 7.4.5 insertadas.")
