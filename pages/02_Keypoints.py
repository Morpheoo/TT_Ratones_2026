...
run_page_splash(
    ...
    subtitle="Cargando extraccion de keypoints...",
)
...
def trim_log_text(lines, max_lines=120):
    if not lines:
        return "[INFO] Aun no hay registros de ejecución."
...
def parse_extract_progress(lines, current_progress):
    ...
    status = "Preparando extracción y post-proceso..."
...
def build_keypoints_main_outputs():
    candidates = [
        ...
        ("Vídeo de validación bbox", st.session_state.get("ultimo_bbox_video")),
        ("Características de SimBA (CSV)", st.session_state.get("ultimo_feature_file")),
        ("Vista previa del HUD", st.session_state.get("ultimo_overlay_path")),
    ]
...
def render_output_panel(snapshot=None):
    ...
    st.caption(
        "Este módulo deja lista la pose cruda, aplica el filtro bbox y sincroniza el puente hacia SimBA. "
        "La vista previa del HUD es una capa de control de calidad y prioriza la pose filtrada si ya existe. "
        "El vídeo multimodal final sigue viviendo en el módulo 04."
    )
    ...
    if is_extract_completed(snapshot) and main_outputs:
        ...
        st.info("Todavía no hay archivos de keypoints generados para este vídeo.")
    ...
    with col_btn1:
        if st.button("Abrir carpeta de keypoints", use_container_width=True, key="btn_open_kp_folder"):
...
def build_render_command():
    ...
    if not pose_path:
        raise FileNotFoundError("No se encontró un archivo de pose de DLC para renderizar.")
...
st.markdown(
    ...
    Utilice DeepLabCut SuperAnimal para procesar el video y generar una vista previa de la capa de inspección.
    """
)
...
if len(_available_projects) > 1:
    ...
    _new_choice = st.selectbox(
        "Destino del proyecto en SimBA",
        options=_available_projects,
        index=_idx,
        format_func=format_project_label,
        help=(
            "Elegí a qué proyecto SimBA escribir las features, ROIs y "
...
if not video_ok:
    st.error("No hay vídeo activo. Selecciónalo primero en el módulo de ingesta de vídeo.")
    st.stop()
...
with col_left:
    ...
    with st.expander("Parámetros de análisis", expanded=True):
        batch_size = st.slider(
            ...
            step=4,
            help="Capado a 32 para no sobrecargar la GPU. En laptop, 16 suele ser el punto más estable.",
        )
        ...
        st.caption("Recomendación: dejar 32 como tope, pero usar 16 por default y subir a 24/32 solo si la GPU se mantiene estable.")
        st.caption("Al terminar DLC, este flujo aplica bbox y deja sincronizado el puente hacia SimBA en automático.")
    ...
    if extraction_running:
        st.info(
            ...
            "esta página reconstruye el progreso leyendo el registro del proceso."
        )
...
with col_right:
    ...
    if st.button(
        ...
        action = "render"
    if hud_disabled:
        st.caption("Primero extrae los keypoints para poder generar la capa de inspección.")
...
if action == "extract":
    ...
    launch_log_viewer_console(extract_snapshot["log_path"], "Registros de extracción de keypoints")
