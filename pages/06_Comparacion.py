...
st.markdown("""
    ...
    **Obligatorio**: De 6 a 8 experimentos por grupo.
""")
...
if df_experiments.empty:
    ...
    st.info("Completa al menos 12 experimentos en el módulo de análisis final para usar esta función.")
    st.stop()
...
5. Presiona el botón **comparar grupos** para generar el análisis.
""")
...
with col_title:
    st.markdown("#### Datos individuales por experimento")
with col_reset:
...
tab1, tab2 = st.tabs([
    "Gráficas de barras con error estándar",
    "Comparación de métricas individual"
])

with tab1:
    st.markdown("##### Comparación de medias con barras de error estándar")
...
def generate_comparison_pdf(df_comparison_table, df_stats, group1_treatment, group2_treatment, n_group1, n_group2):
    ...
    try:
        ...
        # Título
        elements.append(Paragraph("Comparación de grupos", styles['Title']))
...
with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
    ...
    metadata = pd.DataFrame({
        ...
        'Valor': [
            ...
            'Prototipo de análisis automatizado'
        ]
    })
...
with st.expander(" Recomendaciones para pruebas estadísticas"):
    ...
    st.markdown("---")
    st.markdown("**Interpretación del tamaño del efecto con d de Cohen:**")
    st.markdown("""
